...
```{r}
modelo <- inla(
  TOTAL_HOMICIDIOS ~ 1 + POLICIAS + POBLACION + DUMMY +
                      f(MES, model = "rw1") +  
                      f(ESTRATO, model = "rw1") +  
...
```{r}
tabla <- homicidios %>% group_by(COMUNA) %>% tally()
tabla
```
...
```{r}
qnorm(0.05, -0.042459067, 0.07377954)
qnorm(0.95, -0.042459067, 0.07377954)
```
...
```{r}
nuevos_datos$TOTAL_HOMICIDIOS <- NA
modelo_pred <- inla(
  TOTAL_HOMICIDIOS ~ 1 + POLICIAS + POBLACION +
                      ...
                      f(COMUNA_TIEMPO, model = "iid"),
  family = "poisson",
  data = rbind(homicidios, nuevos_datos),
  control.predictor = list(compute = TRUE)
)
```
